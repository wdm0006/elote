...
import logging
from typing import Dict, List, Tuple, Type, Optional, Any, Callable, Union
import time

from elote.arenas.base import BaseArena, History
...
def evaluate_competitor(
    ...
    if progress_callback:
        train_progress = lambda current, total: progress_callback("train", current, total)
    else:
        train_progress = None
    ...
    if progress_callback:
        eval_progress = lambda current, total: progress_callback("eval", current, total)
    else:
        eval_progress = None
